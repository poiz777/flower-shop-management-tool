<?php
	...
	use App\Entity\Personen as Client;
	...
	class ClientForm {
		...
		public function __construct($formEntity=null, ?ShopTranslator $translator=null) {
			...
			$this->formEntity   = $formEntity ? $formEntity : new Client();
					...
					foreach($this->formEntity->getEntityBank() as $fieldName=>$fieldVal){
						if($fieldName == $config['name']){
							if($fieldVal instanceof \DateTime){
								$fieldVal = $fieldVal->format('d.m.Y');   #->format('Y-m-d');
								$fieldVal = preg_match("#^\-#", $fieldVal) ? date("d.m.Y", strtotime('1970-01-01')) : $fieldVal;

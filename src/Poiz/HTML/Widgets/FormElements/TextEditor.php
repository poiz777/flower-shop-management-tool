<?php
	...
	protected  $fieldType  = 'TextEditor';
		/**
		 * Text constructor.
		 *
		 * @param array $config
		...
		protected function buildWidget() {
			$this->build('textarea', true);
			$this->widget  .= $this->addTextEditorInitializerJavascript();
			return $this;
		}
		
		private function addTextEditorInitializerJavascript($height=250){
			return <<<SCRIPT
<script>
    // fullpage  - removed from plugins
    tinymce.init({
		selector: "textarea#{$this->config['id']}",
		height: {$height},
